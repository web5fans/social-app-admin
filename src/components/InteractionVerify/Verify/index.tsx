...
import './index.less';
import IconRefresh from '../assets/refreshIcon.svg?react';
import IconArrowRight from '../assets/arrowIcon.svg?react';
import { useTranslation } from 'react-i18next';
// import RivIcon from '@/components/IconFont';
...
export default memo(
  ({
    ...
    const [tryAgianTips, setTryAgainTips] = useState(false);
    ...
    const getRandomImgSrc = () => {
      ...
      // return `${config?.OSS_BUCKET}static/captcha-bg/${randomN}.png`;
      return `/assets/interactive-image/${randomN}.png`;
    };
    ...
    const reset = () => {
      ...
      blockRef.current.style.left = -1 + 'px';
    ...
    const handleDragMove = (e: any) => {
      ...
      if (moveX < 0 || moveX + 78 >= width) return false;
      setSliderLeft(moveX);
      const blockLeft = ((width - 40) / (width - 40 - 16)) * moveX;
    ...
    return (
      ...
      >
        ...
        >
          <div className="sliderMask" style={{ width: sliderLeft + 10 + 'px' }}>
            <div
              className="slider"
              ...
              onTouchStart={handleDragStart}
            >
              <IconArrowRight className="sliderIcon" />
            </div>
          </div>
          <div className="sliderText">{!tryAgianTips ? defaultTips : t("swipe.tryAgain") }</div>
        </div>
        ...
        >
          {/* <RefreshDefault className="loadingIcon" /> */}
          <span>{t("loading")}</span>
        </div>
      </div>
    );
